...
import duck.develop.calculator.data.source.local.KeyLocalDataSource
...
import duck.develop.calculator.data.source.remote.KeyRemoteDataSource
...
import duck.develop.calculator.data.source.repository.KeyRepository
...
val repository = module {
    ...
    single<KeyDataSource>(named(KEY_REPOSITORY)) {
        KeyRepository(get(named(KEY_LOCAL_DATA_SOURCE)), get(named(KEY_REMOTE_DATA_SOURCE)))
    }
    single<KeyDataSource>(named(KEY_LOCAL_DATA_SOURCE)) { KeyLocalDataSource(get()) }
    single<KeyDataSource>(named(KEY_REMOTE_DATA_SOURCE)) { KeyRemoteDataSource(get()) }
